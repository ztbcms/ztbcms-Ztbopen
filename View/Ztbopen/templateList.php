<Admintemplate file="Common/Head"/>
<body class="J_scroll_fixed">
<div class="wrap J_check_wrap" id="app">
    <div class="mb10">
        <a @click="createBtnEvent" href="javascript:void(0)" class="btn btn-default" title="添加内容"><span
                    class="add"></span>添加内容</a>
    </div>
    <div class="table_list">
        <table width="100%">
            <thead>
            <tr>
                <td align="center">标题</td>
                <td align="center">英文名</td>
                <td align="center">app_id</td>
                <td align="center">模板ID</td>
                <td align="center">描述</td>
                <td align="center">操作</td>
            </tr>
            </thead>
            <tbody>
            <template v-for="item in items">
                <tr>
                    <td align="center">{{ item.title }}</td>
                    <td align="center">{{ item.name }}</td>
                    <td align="center">{{ item.app_id }}</td>
                    <td align="center">{{ item.template_id }}</td>
                    <td align="center">{{ item.description }}</td>
                    <td align="center">
                        <a class="btn btn-primary" href="javascript:;" @click="sendTplMessage(item.id);">测试发送</a>
                        <a href="javascript:;" @click="editBtnEvent(item.id)" class="btn btn-primary">编辑</a>
                        <a href="javascript:;" @click="deleteBtnEvent(item.id)" class="btn btn-danger">删除</a>
                    </td>
                </tr>
            </template>
            </tbody>
        </table>
    </div>

</div>
<script src="{$config_siteurl}statics/js/common.js?v"></script>
<Admintemplate file="Ztbopen/Common/vPageComponnet"/>
<script>
    $(function () {
        new Vue({
            data: {
                app_id: '{:I("app_id", 0)}',
                items: [],
                page: 1,
                limit: 20,
                total_pages: 0,
            },
            el: '#app',
            methods: {
                deleteBtnEvent: function (id) {
                    var that = this;
                    layer.confirm('是否确定删除', {}, function (res) {
                        console.log(id);
                        that.httpPost('{:U("delTemplate")}', {id: id}, function (res) {
                            console.log(res);
                            if (res.status) {
                                location.reload();
                            } else {
                                layer.msg(res.msg);
                            }
                        })
                    });
                },
                editBtnEvent: function (id) {
                    console.log(id);
                    var that = this;
                    layer.open({
                        type: 2,
                        title: '编辑应用',
                        shade: false,
                        area: ['800px', '500px;'],
                        content: "{:U('template')}&app_id="+that.app_id+"&id=" + id,
                    });
                },
                createBtnEvent: function () {
                    var that = this;
                    layer.open({
                        type: 2,
                        title: '新增应用',
                        shade: false,
                        area: ['800px', '500px;'],
                        content: "{:U('template')}&app_id="+that.app_id,
                    });
                },
                getLists: function () {
                    var that = this;

                    this.httpGet("{:U('getTemplateList')}&app_id="+that.app_id, {page: this.page, limit: this.limit}, function (res) {
                        console.log(res);
                        if (res.status) {
                            that.items = res.data
                        }
                    })
                },
                sendTplMessage: function(id){
                    var that = this;
                    layer.open({
                        title: '发送模版消息',
                        type: 2,
                        content: "{:U('sendTplMessage')}&app_id="+that.app_id+'&id='+id,
                        area: ['60%', '80%']
                    });
                }
            },
            mounted: function () {
                console.log('xxx');
                this.getLists();
            }, components: {
                'v-page': pageComponent
            }
        })
    });
</script>
</body>
</html>
